...
def detect(save_img=False):
    ...
    ct1=0
    ct2=0
    save_path,save_name=f"info/{ct2}",''
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            print(f'{s}Done. ({(1E3 * (t2 - t1)):.1f}ms) Inference, ({(1E3 * (t3 - t2)):.1f}ms) NMS {num}')
            # Stream results
            if view_img:
            ...
            int(num)
            if  (frame-nfr>=400 and num>=1) or (num>=1 and (num>nn)):
                save_name = save_path + ".jpg"
                cv2.imwrite(save_name,im0)
                if frame-nfr >= 400 and num >= 1:
                    ct+=1
                if num>=1 and not (num==nn):
                    ct1+=1
                nfr=frame
                ct2+=1
